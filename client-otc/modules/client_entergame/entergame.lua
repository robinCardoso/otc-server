...
function EnterGame.resolveUiPath(name)
  local layout = g_resources.getLayout()
  if layout and layout:len() > 0 then
    local layoutUi = '/layouts/' .. layout .. '/modules/client_entergame/' .. name
    if g_resources.fileExists(layoutUi .. '.otui') then
      return layoutUi
    end
  end
  return name
...
function EnterGame.init()
  ...
  enterGame = g_ui.displayUI(EnterGame.resolveUiPath('entergame'))
...
function EnterGame.terminate()
  ...
  if AccountRegister and AccountRegister.terminate then
    AccountRegister.terminate()
  end
...
function EnterGame.refreshCharacterList()
  ...
  EnterGame.doLogin(G.account, G.password, G.authenticatorToken, G.host)
end
