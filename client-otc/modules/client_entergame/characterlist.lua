...
function CharacterList.init()
  ...
  connect(g_game, { onMapKnown = CharacterList.destroyLoadBox })
...
function CharacterList.terminate()
 if USE_NEW_ENERGAME then return end
  ...
  disconnect(g_game, { onMapKnown = CharacterList.destroyLoadBox })
...
function CharacterList.create(characters, account, otui)
  ...
  charactersWindow = g_ui.displayUI(EnterGame.resolveUiPath(otui))
  ...
  local createBtn = charactersWindow:getChildById('createCharacterButton')
  if createBtn then
    if #characters == 0 then
      createBtn:setImageColor('#ffaa33')
    else
      createBtn:setImageColor('#55aa55')
    end
  end
