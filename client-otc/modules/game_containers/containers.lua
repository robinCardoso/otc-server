...
function onContainerOpen(container, previousContainer)
  ...
  containerWindow.onClose = function()
    modules.game_interface.closeContainerItem(container)
    ...
    elseif mouseButton == MouseButton5 then
      for i, item in ipairs(container:getItems()) do
        if item:isContainer() then
          modules.game_interface.openContainerItem(item)
          return true
        end
      end
    end
  ...
  containerItemWidget:setVirtual(false)
  containerItemWidget.onMouseRelease = function(widget, mousePos, mouseButton)
    if mouseButton == MouseLeftButton or mouseButton == MouseRightButton then
      local containerItem = container:getContainerItem()
      if containerItem then
        modules.game_interface.openContainerItem(containerItem)
        return true
      end
    end
    return false
