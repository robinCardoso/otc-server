...
function bindTurnKey(key, dir)
  ...
  g_keyboard.bindKeyUp(key, function() local player = g_game.getLocalPlayer() if player then player:lockWalk(g_settings.getNumber('walkTurnDelay')) end end, gameRootPanel)
end
...
function onWalkFinish(player)
  ...
  if nextWalkDir ~= nil then
    ...
    nextWalkDir = nil
    walk(dir, 0) -- Executa instantaneamente sem esperar pelo próximo tick do dispatcher
  end
end
...
function walk(dir, ticks) 
  ...
  if not player:canWalk(dir) then -- canWalk return false when previous walk is not finished or not confirmed by server
    if dash then 
      ...
      if ticksToNextWalk < 800 and (lastWalkDir ~= dir or ticks == 0) then
        nextWalkDir = dir
      end
      if ticksToNextWalk < 80 and lastFinishedStep + 500 > g_clock.millis() and nextWalkDir == nil then
        nextWalkDir = dir
      end
      return
    end
  ...
  if firstStepDelay > 0 and firstStep and lastWalkDir == dir and lastWalk + firstStepDelay > g_clock.millis() then
    firstStep = false
  ...
  firstStep = (not player:isWalking() and lastFinishedStep + 50 < g_clock.millis() and walkLock + 50 < g_clock.millis())
  -- Removida a trava artificial de 30ms para emendar passos de forma contínua
  -- if player:isServerWalking() and not dash then
  --   walkLock = math.max(walkLock, g_clock.millis() + 30)
  -- end
  ...
  -- 8.60: tile com criatura (reto ou diagonal) = sem preWalk, sem pacote
  if not g_game.getFeature(GameNewWalking) then
    if not toTile or toTile:blocksCreatureWalk() then
  ...
  -- preWalk so no protocolo novo (GameNewWalking). No 8.60 o offset invertido de isPreWalking
  -- desloca o sprite na direcao OPOSTA antes do passo do servidor.
  if g_game.getFeature(GameNewWalking) and toTile and toTile:isWalkable() then
    if not player:isServerWalking() and not ignoredCanWalk then
      player:preWalk(dir)
      preWalked = true
    end
  elseif not toTile or not toTile:isWalkable() then
  ...
  if player:isServerWalking() and not dash and lastWalkDir ~= dir then
    g_game.stop()
    player:finishServerWalking()
    autoFinishNextServerWalk = g_clock.millis() + 100
  end
