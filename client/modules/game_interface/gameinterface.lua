...
function onGameStart()
  refreshViewMode()
  show()
  -- Evita passo extra no clique (860 nao usa GameNewWalking)
  ...
  if useThing then
    if useThing:isContainer() then
      if useThing:getParentContainer() then
        menu:addOption(tr('Open'), function() openContainerItem(useThing) end, shortcut)
...
function focusContainerWindow(item)
  if not item or not item.getPosition then
    return false
  end
  ...
  for _, container in pairs(g_game.getContainers()) do
    ...
    if containerItem and containerItem:getPosition() then
      local cPos = containerItem:getPosition()
      if cPos.x == itemPos.x and cPos.y == itemPos.y and cPos.z == itemPos.z then
...
function openContainerItem(item)
  if not item then
    return
  end

  if focusContainerWindow(item) then
    return
  end
...
function closeContainerItem(container)
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature, marking)
  ...
  elseif not modules.client_options.getOption('classicControl') then
    ...
    elseif creatureThing and creatureThing:getPosition().z == autoWalkPos.z and g_keyboard.isAltPressed() and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) then
      g_game.attack(creatureThing)
      return true
    elseif useThing and not autoWalkPos and keyboardModifiers == KeyboardNoModifier and mouseButton == MouseLeftButton and useThing:isContainer() then
      openContainerItem(useThing)
      return true
    end
    ...
    if keyboardModifiers == KeyboardNoModifier and mouseButton == MouseRightButton and not g_mouse.isPressed(MouseLeftButton) then
      ...
      -- chão / tile vazio: sem use no ground (evita andar) nem auto-walk
      return true
    elseif lookThing and keyboardModifiers == KeyboardShiftModifier and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) then
      g_game.look(lookThing)
      return true
  ...
  if modules.client_options.getOption('classicControl') and not g_app.isMobile() and mouseButton == MouseRightButton then
    return true
  end
  ...
  if autoWalkPos and keyboardModifiers == KeyboardNoModifier and (mouseButton == MouseLeftButton or mouseButton == MouseTouch2 or mouseButton == MouseTouch3) then
    player:stopAutoWalk()
    -- Caminho calculado em C++ (findPathAsync): contorna mobs em diagonal; falha → onAutoWalkFail
