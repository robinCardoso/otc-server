local defaultOptions = {
  ...
  containerPanel = 8,
  -- Deslize do mapa vem do C++ (creature.cpp); FPS alto nao precisa ser baixo
...
local UI_THEME_LABEL = {
  dark = 'Escuro',
  medium = 'Cinza',
  light = 'Claro',
}

local function normalizeUiTheme(value)
  if type(value) ~= 'string' then
    return 'dark'
  end
  local v = value:lower()
  if v == 'escuro' or v == 'dark' then return 'dark' end
  if v == 'cinza' or v == 'medium' then return 'medium' end
  if v == 'claro' or v == 'light' then return 'light' end
  return 'dark'
end

local function uiThemeLabel(theme)
  return tr(UI_THEME_LABEL[theme] or UI_THEME_LABEL.dark)
end
local optionsButton
local optionsTabBar
local options = {}
...
local walkDefaultKeys = {
  ...
  'walkTeleportDelay', 'walkCtrlTurnDelay'
}

local function applyWalkDefaultsToSavedSettings()
  if g_settings.getNumber('walkSettingsVersion') >= WALK_SETTINGS_VERSION then
    return
  end
  for _, key in ipairs(walkDefaultKeys) do
    local v = defaultOptions[key]
    if type(v) == 'boolean' then
      g_settings.set(key, v)
    else
      g_settings.set(key, tonumber(v))
    end
  end
  g_settings.set('walkSettingsVersion', WALK_SETTINGS_VERSION)
end

-- Mapa/câmera: classic view on (layout OK), FPS limitado para deslize suave
local MAP_SMOOTH_SETTINGS_VERSION = 4
local mapSmoothDefaultKeys = {
  'classicView', 'floorFading', 'vsync', 'backgroundFrameRate',
  'optimizationLevel', 'cacheMap'
}

local function applyMapSmoothDefaultsToSavedSettings()
  if g_settings.getNumber('mapSmoothSettingsVersion') >= MAP_SMOOTH_SETTINGS_VERSION then
    return
  end
  for _, key in ipairs(mapSmoothDefaultKeys) do
    local v = defaultOptions[key]
    if type(v) == 'boolean' then
      g_settings.set(key, v)
    else
  ...
  g_settings.set('mapSmoothSettingsVersion', MAP_SMOOTH_SETTINGS_VERSION)
end
...
function init()
  ...
  applyWalkDefaultsToSavedSettings()
  applyMapSmoothDefaultsToSavedSettings()
  ...
  for _,panel in pairs(optionsTabBar:getTabsPanel()) do
    ...
    if widget then
      ...
      elseif widget:getStyle().__class == 'UIComboBox' then
        if key == 'uiTheme' and type(value) == 'string' then
          widget:setCurrentOption(uiThemeLabel(normalizeUiTheme(value)), true)
          break
        end
        if type(value) == "string" then
