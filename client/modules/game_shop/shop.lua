...
function show()
  ...
  if not g_game.getFeature(GameExtendedOpcode) then
    displayInfoBox(tr('Shop'), tr('Shop requires GameExtendedOpcode on the client and otcv8_shop.lua on the server.\nSee otcv8-dev/docs/SHOP-MODULE.md'))
    return
  end
...
function onCoinBalance(coins, transferableCoins)
  if not shop or otcv8shop then return end
  shop.infoPanel.points:setText(tr("Coins:") .. " " .. tostring(coins))
...
function processStatus(data)
  ...
  if data['points'] ~= nil then
    shop.infoPanel.points:setText(tr("Coins:") .. " " .. tostring(data['points']))
  end
