...
function onLogin(player)
	...
	if player:getLastLoginSaved() <= 0 then
		...
		-- OTC 8.60: sendOutfitWindow no login pode derrubar o tfs.exe (0xC0000005). Cliente OTC tem outfit proprio.
		local client = player:getClient()
		local isOtclient = client and client.os >= 10
		if not isOtclient then
...
            - Commands -]]
   ...
   -- player:sendTextMessage(messageType or MESSAGE_STATUS_CONSOLE_ORANGE, 'Bem vindo ao Order-Global!')
	...
	player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, 'Comandos: !bless, !aol, !shovel, !pick, !rope, !machete, !xp, !power')
