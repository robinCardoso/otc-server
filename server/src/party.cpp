...
bool Party::isPartyAlly(const Player* player) const
{
	if (!player) {
		return false;
	}
...
uint8_t Party::getVocationBonusPercent() const
{
	return static_cast<uint8_t>(extraExpRate * 100.f);
}

void Party::broadcastVocationBonus()
{
	...
	const size_t differentVocations = vocationIds.size();
	if (differentVocations <= 1) {
		return;
	}

	std::ostringstream ss;
	ss << "Party experience bonus: +" << static_cast<uint32_t>(getVocationBonusPercent()) << "% ("
	   << differentVocations << " different vocations).";
	broadcastPartyMessage(MESSAGE_INFO_DESCR, ss.str(), true);
}
