...
{
	using Allocator = LockfreePoolingAllocator<OutputMessage, OUTPUTMESSAGE_FREE_LIST_CAPACITY>;
	Allocator allocator;
	OutputMessage* ptr = allocator.allocate(1);
	new (ptr) OutputMessage();
	return std::shared_ptr<OutputMessage>(ptr, [allocator](OutputMessage* p) mutable {
		p->~OutputMessage();
