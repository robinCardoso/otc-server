...
class Spells final : public BaseEvents
{
	public:
		...
		const std::map<uint16_t, RuneSpell*>& getRunes() const {
			return runes;
		}
...
class Spell : public BaseSpell
{
	public:
		...
		bool linkScriptCombat();
		Combat* getLinkedCombat() const {
			return linkedCombat;
		}
		SpellGroup getGroup() const {
			return spellGroup;
		}
		static uint8_t spellGroupToClientId(SpellGroup group);
	...
	protected:
		...
		SpellGroup spellGroup = SpellGroup::Support;
		Combat* linkedCombat = nullptr;
