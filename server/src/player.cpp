	...
	if (isLogin && creature == this) {
		...
		if (guild) {
			if (g_config.getBoolean(ConfigManager::ENABLE_TFS_DIAGNOSTIC_LOG)) {
				std::cout << "> [login] guild->addMember" << std::endl;
			}
		...
		if (g_config.getBoolean(ConfigManager::ENABLE_TFS_DIAGNOSTIC_LOG)) {
			std::cout << "> [login] checkPlayersRecord" << std::endl;
		}
		g_game.checkPlayersRecord();
		if (g_config.getBoolean(ConfigManager::ENABLE_TFS_DIAGNOSTIC_LOG)) {
			std::cout << "> [login] updateOnlineStatus" << std::endl;
		}
		IOLoginData::updateOnlineStatus(guid, true);
		if (g_config.getBoolean(ConfigManager::ENABLE_TFS_DIAGNOSTIC_LOG)) {
			std::cout << "> [login] onCreatureAppear done" << std::endl;
		}
	}
}
...
ReturnValue Player::queryAdd(int32_t index, const Thing& thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	// Moedas e itens sem slot/arma (slotPosition 0, WEAPON_NONE) nao vao para mao/capacete/etc.
	if (slotPosition == 0 && item->getWeaponType() == WEAPON_NONE) {
...
void Player::doAttacking(uint32_t)
{
	...
	if ((OTSYS_TIME() - lastAttack) >= getAttackSpeed()) {
		...
		if (weapon) {
			const WeaponType_t weaponType = tool ? tool->getWeaponType() : WEAPON_NONE;
			const bool kiteWeapon = weapon->interruptSwing() || weaponType == WEAPON_WAND;

			if (!weapon->interruptSwing()) {
				result = weapon->useWeapon(this, tool, attackedCreature);
			} else if (kiteWeapon || canDoAction() || classicSpeed) {
				result = weapon->useWeapon(this, tool, attackedCreature);
			} else {
		...
		const Weapon* schedWeapon = weapon;
		const Item* schedTool = tool;
		const bool kiteSched = schedWeapon && (schedWeapon->interruptSwing()
			|| (schedTool && schedTool->getWeaponType() == WEAPON_WAND));
		if (!classicSpeed && schedWeapon && !kiteSched) {
...
void Player::onPlacedCreature()
{
	...
	if (g_config.getBoolean(ConfigManager::ENABLE_TFS_DIAGNOSTIC_LOG)) {
		std::cout << "> [login] Lua onLogin start: " << name << std::endl;
	}
	...
	if (g_config.getBoolean(ConfigManager::ENABLE_TFS_DIAGNOSTIC_LOG)) {
		std::cout << "> [login] Lua onLogin end: " << name << std::endl;
		std::cout << "> [login] ready for game packets: " << name << std::endl;
	}
}
...
bool Player::isPartyAlly(const Player* player) const
{
	if (!player || !party) {
		return false;
	}
	return party->isPartyAlly(player);
}

bool Player::isInPartyPkMode() const
{
	return skull == SKULL_WHITE || skull == SKULL_RED || skull == SKULL_BLACK;
}
