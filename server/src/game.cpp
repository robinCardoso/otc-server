...
slots_t getEquipSlotType(const ItemType& it)
{
...
Item* searchEquipItem(Container* container, uint16_t itemId)
{
...
Item* findPlayerEquipItem(Player* player, uint16_t itemId)
{
	for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_AMMO; ++slot) {
		...
		if (Container* container = item->getContainer()) {
			if (Item* found = searchEquipItem(container, itemId)) {
				return found;
			}
		}
	}
...
void Game::playerEquipItem(uint32_t playerId, uint16_t spriteId, uint8_t count)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
	...
	const slots_t targetSlot = getEquipSlotType(it);
	Item* item = findPlayerEquipItem(player, it.id);
	if (!item) {
		player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
		return;
	}
	...
	Position toPos(0xFFFF, 0, 0);
	toPos.y = static_cast<uint16_t>(targetSlot);

	player->setNextActionTask(nullptr);

	Item* currentSlotItem = player->getInventoryItem(targetSlot);
	if (currentSlotItem && currentSlotItem == item) {
		...
		Container* backpack = backpackItem->getContainer();
		if (!backpack) {
			player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
			return;
		}

		if (!g_events->eventPlayerOnMoveItem(player, item, count, fromPos, toPos, fromCylinder, backpack)) {
			return;
		}

		ReturnValue ret = internalMoveItem(player, backpack, INDEX_WHEREEVER, item, count, nullptr, 0, player);
		if (ret != RETURNVALUE_NOERROR) {
			player->sendCancelMessage(ret);
		}
		return;
	...
	if (!g_events->eventPlayerOnMoveItem(player, item, count, fromPos, toPos, fromCylinder, player)) {
		return;
	}

	ReturnValue ret = internalMoveItem(fromCylinder, player, static_cast<int32_t>(targetSlot), item, count, nullptr, 0, player);
	if (ret != RETURNVALUE_NOERROR) {
		player->sendCancelMessage(ret);
	}
}
						 ...
						 uint8_t index, uint16_t spriteId)
{
	...
	if (!player->canDoAction()) {
		if (pos.x == 0xFFFF && item->getContainer()) {
			player->setNextActionTask(nullptr);
			player->resetIdleTime();
							   ...
							   bool ghostMode, SpectatorHashSet* spectatorsPtr/* = nullptr*/, const Position* pos/* = nullptr*/)
{
	...
	if (!spectatorsPtr || spectatorsPtr->empty()) {
		...
		if (type != TALKTYPE_YELL && type != TALKTYPE_MONSTER_YELL) {
			...
			map.getSpectators(spectators, *pos, true, false, Map::clientMapWidth, Map::clientMapWidth, Map::clientMapHeight, Map::clientMapHeight);
		}
	} else {
...
void Game::parsePlayerExtendedOpcode(uint32_t playerId, uint8_t opcode, const std::string& buffer)
{
	...
	if (diagnosticLog) {
		std::cout << "> [extopcode] " << player->getName() << " opcode=" << static_cast<unsigned>(opcode)
		          << " len=" << buffer.size() << std::endl;
	}
