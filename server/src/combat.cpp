	...
	if (attacker) {
		if (const Player* targetPlayer = target->getPlayer()) {
			...
			if (const Player* attackerPlayer = attacker->getPlayer()) {
				...
				if (attackerPlayer->isPartyAlly(targetPlayer) && !attackerPlayer->isInPartyPkMode() && !targetPlayer->isInPartyPkMode()) {
					return RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER;
				}
			...
			if (attacker->isSummon()) {
				if (const Player* masterAttackerPlayer = attacker->getMaster()->getPlayer()) {
					...
					if (masterAttackerPlayer->isPartyAlly(targetPlayer) && !masterAttackerPlayer->isInPartyPkMode() && !targetPlayer->isInPartyPkMode()) {
						return RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER;
					}
				}
			}
...
bool ValueCallback::getPreviewValues(Player* player, int32_t& minValue, int32_t& maxValue) const
{
	minValue = 0;
	maxValue = 0;
	if (!player || !scriptInterface) {
		return false;
	}

	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - ValueCallback::getPreviewValues] Call stack overflow" << std::endl;
		return false;
	}
	...
	lua_State* L = scriptInterface->getLuaState();
	if (!scriptInterface->pushFunction(scriptId)) {
		scriptInterface->resetScriptEnv();
		return false;
	}
	...
	if (!lua_isnumber(L, -2) || !lua_isnumber(L, -1)) {
		LuaScriptInterface::reportError(nullptr, "getPreviewValues callback must return two numbers");
		lua_pop(L, 2);
		scriptInterface->resetScriptEnv();
		return false;
	}

	minValue = LuaScriptInterface::getNumber<int32_t>(L, -2);
	maxValue = LuaScriptInterface::getNumber<int32_t>(L, -1);
	lua_pop(L, 2);
...
bool Combat::getPreviewValues(Player* player, int32_t& minValue, int32_t& maxValue) const
{
	minValue = 0;
	maxValue = 0;
	if (!player) {
	...
	if (params.valueCallback) {
		return params.valueCallback->getPreviewValues(player, minValue, maxValue);
	}

	if (formulaType == COMBAT_FORMULA_LEVELMAGIC) {
		const int32_t levelFormula = player->getLevel() * 2 + player->getMagicLevel() * 3;
		minValue = static_cast<int32_t>(levelFormula * mina + minb);
		maxValue = static_cast<int32_t>(levelFormula * maxa + maxb);
		return true;
	}
	...
	if (formulaType == COMBAT_FORMULA_SKILL) {
		...
		const Weapon* weapon = g_weapons->getWeapon(tool);
		if (weapon && tool) {
			minValue = weapon->getWeaponDamage(player, nullptr, tool, false);
			maxValue = weapon->getWeaponDamage(player, nullptr, tool, true);
			return true;
		}
		minValue = static_cast<int32_t>(minb);
		maxValue = static_cast<int32_t>(maxb);
		return true;
	}

	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		minValue = static_cast<int32_t>(mina);
		maxValue = static_cast<int32_t>(maxa);
		return true;
	}
