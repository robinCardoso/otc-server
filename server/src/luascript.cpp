...
void LuaScriptInterface::registerFunctions()
{
	...
	registerMethod("Player", "getCombatPreview", LuaScriptInterface::luaPlayerGetCombatPreview);
...
int LuaScriptInterface::luaPlayerGetSpectators(lua_State* L)
{
	...
	lua_pushnumber(L, static_cast<lua_Number>(spectators.size()));
	return 2;
}
...
int LuaScriptInterface::luaPlayerGetCombatPreview(lua_State* L)
{
	// player:getCombatPreview()
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	const bool diagnosticLog = g_config.getBoolean(ConfigManager::ENABLE_TFS_DIAGNOSTIC_LOG);
	if (diagnosticLog) {
		std::cout << "> [combatpreview] " << player->getName() << " getCombatPreview: begin" << std::endl;
	}
	// #region agent log
	CombatPreview::agentDebugLog1ecf01("H0", "luascript.cpp:getCombatPreview", "entry", player->getID(), player->getVocationId());
	// #endregion

	static const char* fightModeNames[] = {"", "attack", "balanced", "defense"};

	const Item* shieldItem = nullptr;
	const Item* weaponConst = nullptr;
	player->getShieldAndWeapon(shieldItem, weaponConst);
	Item* weaponItem = const_cast<Item*>(weaponConst);
	if (weaponItem && !CombatPreview::isValidCombatWeaponItem(weaponItem)) {
		weaponItem = nullptr;
	}
	if (shieldItem && shieldItem->getWeaponType() != WEAPON_SHIELD) {
		shieldItem = nullptr;
	}
	// #region agent log
	CombatPreview::agentDebugLog1ecf01(
		"H-A",
		"luascript.cpp:after_getShieldAndWeapon",
		"shield_weapon_ids",
		shieldItem ? shieldItem->getID() : 0,
		weaponItem ? weaponItem->getID() : 0);
	// #endregion

	const Weapon* weaponTool = nullptr;
	int32_t damageModifier = 100;
	if (weaponItem) {
		weaponTool = g_weapons->getWeapon(weaponItem);
		// #region agent log
		CombatPreview::agentDebugLog1ecf01(
			"H-B",
			"luascript.cpp:after_getWeapon",
			"weapon_tool",
			weaponItem->getID(),
			weaponTool ? 1u : 0u);
		// #endregion
		if (weaponTool) {
			// Preview has no combat target; nullptr crashes in playerWeaponCheck (target->getPosition).
			damageModifier = weaponTool->playerWeaponCheck(player, player, weaponItem->getShootRange());
			// #region agent log
			CombatPreview::agentDebugLog1ecf01("H-B", "luascript.cpp:after_playerWeaponCheck", "damage_modifier", static_cast<uint32_t>(damageModifier), 0);
			// #endregion
		}
	}

	std::string attackKind = "fist";
	std::string weaponName = "fist";
	const char* weaponTypeName = nullptr;
	...
	if (weaponItem && weaponTool) {
		// #region agent log
		CombatPreview::agentDebugLog1ecf01("H-C", "luascript.cpp:weapon_branch", "enter_weapon_damage", weaponItem->getID(), 0);
		// #endregion
		attackSkill = player->getWeaponSkill(weaponItem);
		attackValue = weaponItem->getAttack();
		weaponName = weaponItem->getName();

		const WeaponType_t weaponType = player->getWeaponType();
		attackKind = CombatPreview::resolveAttackDisplayKind(weaponType, weaponItem);
		switch (weaponType) {
			case WEAPON_DISTANCE:
				weaponTypeName = "distance";
				break;
			case WEAPON_WAND:
				weaponTypeName = (attackKind == "rod") ? "rod" : "wand";
				break;
			case WEAPON_SWORD:
		...
		WeaponDamagePreview damagePreview;
		if (CombatPreview::computeWeaponDamagePreview(player, weaponTool, weaponItem, weaponType, damageModifier, damagePreview)) {
			minDamage = damagePreview.minDamage;
			maxDamage = damagePreview.maxDamage;
			minVsPlayer = damagePreview.minVsPlayer;
			minVsMonster = damagePreview.minVsMonster;
		}
		// #region agent log
		CombatPreview::agentDebugLog1ecf01("H-C", "luascript.cpp:after_computeWeaponDamagePreview", "preview_damage",
			static_cast<uint32_t>(minDamage), static_cast<uint32_t>(maxDamage));
		// #endregion
		elementMax = weaponTool->getElementDamage(player, nullptr, weaponItem);
		// #region agent log
	...
	if (diagnosticLog) {
		std::cout << "> [combatpreview] " << player->getName() << " getCombatPreview: weapon done" << std::endl;
	}

	lua_createtable(L, 0, 16);

	setField(L, "vocation", player->getVocationId());
	setField(L, "level", player->getLevel());
	setField(L, "maglevel", player->getMagicLevel());
	setField(L, "mana", player->getMana());
	setField(L, "maxMana", player->getMaxMana());

	const char* fightMode = (player->fightMode >= 1 && player->fightMode <= 3) ? fightModeNames[player->fightMode] : "attack";
	setField(L, "fightMode", fightMode);
	setField(L, "attackFactor", player->getAttackFactor());
	setField(L, "damageModifier", damageModifier);
	...
	if (weaponTypeName) {
		setField(L, "weaponType", weaponTypeName);
	}
	setField(L, "skill", attackSkill);
	setField(L, "attackValue", attackValue);
	setField(L, "min", minDamage);
	setField(L, "max", maxDamage);
	setField(L, "minVsPlayer", minVsPlayer);
	setField(L, "minVsMonster", minVsMonster);
	...
	if (shieldItem) {
		...
		setField(L, "shieldName", "");
		setField(L, "shieldDefense", 0);
	}
	lua_setfield(L, -2, "attack");

	static const std::pair<slots_t, const char*> equipmentSlots[] = {
	...
	for (const auto& slotEntry : equipmentSlots) {
		Item* item = player->getInventoryItem(slotEntry.first);
		if (!item || item->isRemoved()) {
			continue;
		}
		if (!CombatPreview::isValidEquipmentSlotItem(item, slotEntry.first)) {
			continue;
		}
		...
		setField(L, "slot", slotEntry.second);
		setField(L, "name", item->getName());
		setField(L, "serverItemId", item->getID());
		setField(L, "clientId", item->getClientID());
		setField(L, "armor", item->getArmor());
		lua_rawseti(L, -2, ++equipmentIndex);
	}
	lua_setfield(L, -2, "equipment");

	if (diagnosticLog) {
		std::cout << "> [combatpreview] " << player->getName() << " getCombatPreview: equipment done" << std::endl;
	}

	CombatPreview::pushSpellPreviews(L, player);
	...
	if (diagnosticLog) {
		std::cout << "> [combatpreview] " << player->getName() << " getCombatPreview: done" << std::endl;
	}
