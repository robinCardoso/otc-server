...
bool Spell::configureSpell(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("group"))) {
		const std::string groupName = attr.as_string();
		if (strcasecmp(groupName.c_str(), "attack") == 0) {
			spellGroup = SpellGroup::Attack;
		} else if (strcasecmp(groupName.c_str(), "healing") == 0) {
			spellGroup = SpellGroup::Healing;
		} else {
			spellGroup = SpellGroup::Support;
		}
	}
...
const char* Spell::getGroupName() const
{
	switch (spellGroup) {
		case SpellGroup::Attack:
			return "attack";
		case SpellGroup::Healing:
			return "healing";
		default:
			return "support";
	}
...
uint8_t Spell::spellGroupToClientId(SpellGroup group)
{
	switch (group) {
		case SpellGroup::Attack:
			return 1;
...
bool Spell::linkScriptCombat()
{
	linkedCombat = g_luaEnvironment.getLastLoadedCombat();
	return linkedCombat != nullptr;
}
...
void Spell::postCastSpell(Player* player, bool finishedCast /*= true*/, bool payCost /*= true*/) const
{
	if (finishedCast) {
		if (!player->hasFlag(PlayerFlag_HasNoExhaustion)) {
			...
			if (groupCooldown > 0) {
				player->sendSpellGroupCooldown(spellGroupToClientId(spellGroup), groupCooldown);
			}
