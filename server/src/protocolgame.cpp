...
bool ProtocolGame::canSee(int32_t x, int32_t y, int32_t z) const
{
	...
	if ((x >= myPos.getX() - Map::clientMapLeft + offsetz) && (x <= myPos.getX() + Map::clientMapRight + offsetz) &&
	        (y >= myPos.getY() - Map::clientMapTop + offsetz) && (y <= myPos.getY() + Map::clientMapBottom + offsetz)) {
		return true;
	}
	return false;
}
...
void ProtocolGame::parseEquipItem(NetworkMessage& msg)
{
	...
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerEquipItem, player->getID(), spriteId, count);
}
...
void ProtocolGame::sendSpellGroupCooldown(uint8_t groupId, uint32_t time)
{
	NetworkMessage msg;
	msg.addByte(0xA5);
	msg.addByte(groupId);
...
void ProtocolGame::sendMapDescription(const Position& pos)
{
	...
	GetMapDescription(pos.x - Map::clientMapLeft, pos.y - Map::clientMapTop, pos.z, Map::clientMapWidth, Map::clientMapHeight, msg);
	writeToOutputBuffer(msg);
}
	...
	if (creature == player) {
		if (oldStackPos >= 10) {
			...
			if (oldPos.y > newPos.y) { // north, for old x
				msg.addByte(0x65);
				GetMapDescription(oldPos.x - Map::clientMapLeft, newPos.y - Map::clientMapTop, newPos.z, Map::clientMapWidth, 1, msg);
			} else if (oldPos.y < newPos.y) { // south, for old x
				msg.addByte(0x67);
				GetMapDescription(oldPos.x - Map::clientMapLeft, newPos.y + Map::clientMapBottom, newPos.z, Map::clientMapWidth, 1, msg);
			}

			if (oldPos.x < newPos.x) { // east, [with new y]
				msg.addByte(0x66);
				GetMapDescription(newPos.x + Map::clientMapRight, newPos.y - Map::clientMapTop, newPos.z, 1, Map::clientMapHeight, msg);
			} else if (oldPos.x > newPos.x) { // west, [with new y]
				msg.addByte(0x68);
				GetMapDescription(newPos.x - Map::clientMapLeft, newPos.y - Map::clientMapTop, newPos.z, 1, Map::clientMapHeight, msg);
			}
			writeToOutputBuffer(msg);
		}
...
void ProtocolGame::sendOutfitWindow()
{
	...
	protocolOutfits.reserve(outfits.size() + 4);
	if (player->isAccessPlayer()) {
...
void ProtocolGame::MoveUpCreature(NetworkMessage& msg, const Creature* creature, const Position& newPos, const Position& oldPos)
{
	...
	if (newPos.z == 7) {
		int32_t skip = -1;
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, 5, Map::clientMapWidth, Map::clientMapHeight, 3, skip); //(floor 7 and 6 already set)
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, 4, Map::clientMapWidth, Map::clientMapHeight, 4, skip);
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, 3, Map::clientMapWidth, Map::clientMapHeight, 5, skip);
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, 2, Map::clientMapWidth, Map::clientMapHeight, 6, skip);
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, 1, Map::clientMapWidth, Map::clientMapHeight, 7, skip);
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, 0, Map::clientMapWidth, Map::clientMapHeight, 8, skip);
	...
	//underground, going one floor up (still underground)
		...
		int32_t skip = -1;
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, oldPos.getZ() - 3, Map::clientMapWidth, Map::clientMapHeight, 3, skip);
	...
	//west
	msg.addByte(0x68);
	GetMapDescription(oldPos.x - Map::clientMapLeft, oldPos.y - (Map::clientMapTop - 1), newPos.z, 1, Map::clientMapHeight, msg);

	//north
	msg.addByte(0x65);
	GetMapDescription(oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, newPos.z, Map::clientMapWidth, 1, msg);
}
...
void ProtocolGame::MoveDownCreature(NetworkMessage& msg, const Creature* creature, const Position& newPos, const Position& oldPos)
{
	...
	if (newPos.z == 8) {
		...
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, newPos.z, Map::clientMapWidth, Map::clientMapHeight, -1, skip);
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, newPos.z + 1, Map::clientMapWidth, Map::clientMapHeight, -2, skip);
		GetFloorDescription(msg, oldPos.x - Map::clientMapLeft, oldPos.y - Map::clientMapTop, newPos.z + 2, Map::clientMapWidth, Map::clientMapHeight, -3, skip);
	...
	//east
	msg.addByte(0x66);
	GetMapDescription(oldPos.x + Map::clientMapRight, oldPos.y - (Map::clientMapTop + 1), newPos.z, 1, Map::clientMapHeight, msg);

	//south
	msg.addByte(0x67);
	GetMapDescription(oldPos.x - Map::clientMapLeft, oldPos.y + Map::clientMapBottom, newPos.z, Map::clientMapWidth, 1, msg);
}
